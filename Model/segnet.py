...
class DecoderBottleneck_segnet(nn.Module):
    def __init__(self, inchannels):
        super(DecoderBottleneck_segnet, self).__init__()
...
class LastBottleneck_segnet(nn.Module):
    def __init__(self, inchannels):
        super(LastBottleneck_segnet, self).__init__()
        self.conv1 = nn.Conv2d(inchannels, inchannels//4, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(inchannels//4)
...
class SegResNet(BaseModel):
    def __init__(self, num_classes, in_channels=3, pretrained=True, freeze_bn=False, **_):
        ...
        for i, block in enumerate(resnet50_blocks[:-1]):
            ...
            decoder.append(nn.Sequential(*new_block, DecoderBottleneck_segnet(channels[i])))
        new_block = list(resnet50_blocks[-1].children())[::-1][:-1]
        decoder.append(nn.Sequential(*new_block, LastBottleneck_segnet(256)))
