...
class ToTensor(object):
    def __call__(self, cvimage, boxes=None, labels=None,masks=None):
        if len(cvimage.shape)==3:
            if cvimage.max()>10:
                return torch.from_numpy(cvimage.copy().astype(np.float32))[:,:,:3].permute(2, 0, 1)/255.0, torch.from_numpy(boxes.copy()).float(), torch.from_numpy(labels.copy()).long(),torch.from_numpy(masks.copy()).long()
            else:
            ...
            if cvimage.max()>10:
                return (torch.from_numpy(cvimage.copy())[:,:,]/255.0).unsqueeze(0), torch.from_numpy(boxes.copy()).float(), torch.from_numpy(labels.copy()).long(),torch.from_numpy(masks.copy())
