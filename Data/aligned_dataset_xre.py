...
# A_numpy = np.load(r'D:\cov\program\Data\russia\train.npy')
# B_numpy = np.load(r'D:\cov\program\Data\russia\mask.npy')
...
#         At,_,_,Bt = self.transform(A,np.array([[0,0,1,1]]),np.array([[4]]),B) 
#         Bt = torch.tensor(np.array(B,dtype=np.float32))
#         return {'A': At, 'B': (Bt.unsqueeze(0).float()>100).float(), 'A_paths': A_path, 'B_paths': B_path}
#         return {'A': At, 'B': Bt.unsqueeze(0).float()/255.0,'A_paths': A_path, 'B_paths': B_path}
        return {'A': At, 'B': (Bt.unsqueeze(0)//255).int(),'A_paths': A_path, 'B_paths': B_path}
